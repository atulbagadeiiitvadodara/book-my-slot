import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuthStore } from '../store/authStore';
import { getGoogleAuthUrl } from '../api/auth';

export default function LoginPage() {
  const { user, isLoading } = useAuthStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (!isLoading && user) navigate('/dashboard');
  }, [user, isLoading]);

  const handleGoogleLogin = () => {
    window.location.href = getGoogleAuthUrl();
  };

  if (isLoading) return null;

  return (
    <div className="min-h-screen bg-bg-primary flex items-center justify-center p-4">
      {/* Background grid */}
      <div
        className="fixed inset-0 opacity-[0.03] pointer-events-none"
        style={{
          backgroundImage: 'linear-gradient(#6c63ff 1px, transparent 1px), linear-gradient(90deg, #6c63ff 1px, transparent 1px)',
          backgroundSize: '48px 48px',
        }}
      />

      <div className="w-full max-w-sm relative z-10">
        {/* Logo */}
        <div className="flex items-center justify-center gap-2.5 mb-8">
          <div className="w-10 h-10 bg-accent rounded-xl flex items-center justify-center">
            <span className="text-white font-mono font-bold text-lg">S</span>
          </div>
          <span className="font-sans font-medium text-text-primary text-xl">SlotSync</span>
        </div>

        {/* Card */}
        <div className="bg-bg-secondary border border-border rounded-2xl p-7">
          <h1 className="text-xl font-medium text-text-primary text-center mb-1.5">Welcome back</h1>
          <p className="text-sm text-text-muted text-center mb-6 leading-relaxed">
            Sign in to manage your schedule and share your availability.
          </p>

          {/* Feature pills */}
          <div className="flex flex-wrap gap-2 justify-center mb-6">
            {['Free forever', 'No credit card', '2-min setup'].map(f => (
              <span key={f} className="text-[11px] bg-bg-tertiary border border-border-light text-text-faint px-3 py-1 rounded-full">
                {f}
              </span>
            ))}
          </div>

          <button
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-800 rounded-xl py-3 px-4 text-sm font-medium transition-colors"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </button>

          <div className="mt-5 pt-5 border-t border-border-light">
            <p className="text-[11px] text-text-faint text-center leading-relaxed">
              By signing in, you agree to our{' '}
              <span className="text-accent cursor-pointer hover:underline">Terms</span>
              {' '}and{' '}
              <span className="text-accent cursor-pointer hover:underline">Privacy Policy</span>.
            </p>
          </div>
        </div>

        <p className="text-center text-[11px] text-text-faint mt-5">
          Trusted by thousands of professionals worldwide.
        </p>
      </div>
    </div>
  );
}
